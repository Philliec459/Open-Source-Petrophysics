...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log_canvas.setMinimumHeight(300)
        self.log_canvas.setMaximumHeight(850)
        ...
        self.project_dock = self._dock("Project", self.project_tree)
        self.addDockWidget(Qt.LeftDockWidgetArea, self.project_dock)
    
        self.curve_inventory_dock = self._dock(
            "Curve Inventory",
            self._placeholder("Later: families + missing curves")
        )
        self.addDockWidget(Qt.LeftDockWidgetArea, self.curve_inventory_dock)
    
        self.align_panel = self._build_alignment_panel()
        self.align_dock = self._dock("Alignment", self.align_panel)
        self.addDockWidget(Qt.RightDockWidgetArea, self.align_dock)
    
        self.merge_dock = self._dock(
            "Merge",
            self._placeholder("Merge params + preview toggles")
        )
        self.addDockWidget(Qt.RightDockWidgetArea, self.merge_dock)
    
        self.export_dock = self._dock(
            "Export",
            self._placeholder("Export options")
        )
        ...
        # -----------------------------
        self.resizeDocks(
            [self.project_dock, self.curve_inventory_dock, self.align_dock, self.merge_dock, self.export_dock],
            [220, 220, 280, 260, 220],
            Qt.Horizontal
        )
