...
class PlotsPanel(QWidget):
    ...
    def _build_tracks(self):
        ...
        dt  = self.tracks.addPlot(row=0, col=2, title="Rt")

        nmr = self.tracks.addPlot(row=0, col=3, title="NMR")


        por.setYLink(gr)
        dt.setYLink(gr)    # ✅ SAFE now because we are NOT using log axis
        nmr.setYLink(gr)



        
        for p in (gr, por, dt, nmr):
            p.invertY(True)
        ...
        self.track_items = {"gr": gr, "por": por, "dt": dt, "nmr": nmr}
    ...
    def update_depth_plot(self, state):
        ...
        dtco_curve = get_curve("rt_curve", "AT90", fallbacks=["AT90", "AF90"])
        ...
        # -------------------------
        plot("dt", arr(dtco_curve),  pg.mkPen("b", width=1), xlim=(50, 110), invert_x=True)
        plot("dt", arr(rt_curve),  pg.mkPen("r", width=1), xlim=(0.2, 20), invert_x=False)
        ...
        # -------------------------
        plot("nmr", arr(cbw_curve),  pg.mkPen("r", width=1), xlim=(0.0, 0.30), invert_x=True)
