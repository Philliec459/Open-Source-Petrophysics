...
SW_PARAM_SPEC = {
    ...
    "sw.rw": dict(default=0.02, mn=0.001, mx=1.000, step=0.001, dec=4, label="Rw"),
...
class SwPanel(QWidget):
    ...
    def calculate_sw_archie(self):
        ...
        rw = float(params.get("sw.rw", 0.02))
    ...
    def calculate_sw_old(self):
        ...
        rt_curve = picks.get("rt_curve", "").strip()
        vsh_curve = picks.get("vsh_curve", "").strip()
        #qv_curve = picks.get("qv_curve", "").strip()
        cbw_curve = picks.get("cbw_curve", "").strip()
        ...
        #qv = None
        #if qv_curve and qv_curve in subset.columns:
        #    qv = pd.to_numeric(subset[qv_curve], errors="coerce")
        ...
        # Base Archie with Vsh correction
        rt_eff = rt * (1.0 + mslope * vsh)
        sw_archie = ((rw) / (rt_eff * (phit ** m))) ** (1.0 / n)
    
        # Optional mild conductivity correction
        '''
        #if qv is not None and qv.notna().sum() > 0 and B > 0.0:
        #    qv = qv.clip(lower=0.0)
        #    sw = sw_archie * (1.0 + 0.10 * B * qv)
        #    cbw_used = None
        '''
