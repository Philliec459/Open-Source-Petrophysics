...
class PlotsPanelFinal(QWidget):
    ...
    def _refresh_final_depth_plot(self):
        ...
        if (x_phit_nmr is not None) and (x_phie_nmr is not None) and (x_bvie is not None):
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phitn = np.clip(x_phit_nmr[m].astype(float), 0.0, 0.30)
                phien = np.minimum(np.clip(x_phie_nmr[m].astype(float), 0.0, 0.30), phitn)
                bvien = np.minimum(np.clip(x_bvie[m].astype(float), 0.0, 0.30), phien)
