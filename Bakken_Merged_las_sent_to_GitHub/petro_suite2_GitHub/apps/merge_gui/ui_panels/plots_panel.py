...
def make_dspin(minv, maxv, step, decimals, value):
    ...
    # Key: avoid half-typed values / require commit (Enter or focus out)
...
class PlotsPanel(QWidget):
    ...
    def _build_nd_tab(self):
        ...
        btn_vsh = QPushButton("2) Compute Vsh (GR/ND/DT) + HL")
    ...
    def _build_sw_tab(self):
        ps = getattr(self, "param_store", None)
        if ps is None:
            cbw_int = 0.15
        else:
        ...
        cbw_int = float(ps.get("cbw_intercept", 0.15))  # ✅ define it BEFORE use
