...
def make_dspin(minv, maxv, step, decimals, value):
    sb = QDoubleSpinBox()
...
class ParamStore:
    ...
    """
    def __init__(self, controller, org="petro_suite2", app="merge_gui"):
...
class PlotsPanel(QWidget):
    def __init__(self, controller):
        ...
        # -------------------------
        self.tabs.addTab(self._build_nd_tab(), "N–D Crossplot")
        self.tabs.addTab(self._build_gr_tab(), "GR Histogram")
        self.tabs.addTab(self._build_dt_tab(), "DTCO–RHOB Crossplot")
    ...
    def _set_param_and_refresh(self, key, value):
        ...
        # ✅ If SW/M* parameters change, recompute derived curves so plots update immediately
        if key.startswith("sw."):
            # avoid recomputing for mslope-only changes if you want
            if key in ("sw.rw", "sw.B", "sw.m_cem", "sw.n_sat"):
                self._on_compute_sw_clicked()   # this writes BVW/SWT/BVO/MSTAR_APP into analysis_df
    ...
    def _on_compute_vsh_clicked(self):
        ...
        # ---- Hodges–Lehmann combine (median of pairwise averages)
        cols = [c for c in ["VSH_GR", "VSH_ND", "VSH_DT"] if c in df.columns]
        if len(cols) == 1:
            df["VSH_HL"] = pd.to_numeric(df[cols[0]], errors="coerce")
        elif len(cols) == 2:
            a = pd.to_numeric(df[cols[0]], errors="coerce").to_numpy(float)
            b = pd.to_numeric(df[cols[1]], errors="coerce").to_numpy(float)
            df["VSH_HL"] = 0.5 * (a + b)
        else:
            a, b, c = (pd.to_numeric(df[col], errors="coerce").to_numpy(float) for col in cols)
            P = np.stack([0.5 * (a + b), 0.5 * (a + c), 0.5 * (b + c)], axis=0)
            df["VSH_HL"] = np.nanmedian(P, axis=0)

        df["VSH_HL"] = np.clip(pd.to_numeric(df["VSH_HL"], errors="coerce").to_numpy(float), 0.0, 1.0)

        # after VSH_HL computed
        if "VSH_GR" in df.columns:
            df["VSH_HL"] = df["VSH_HL"].fillna(df["VSH_GR"])
    ...
    def _on_compute_ws_clicked(self):
        ...
        # --- Resolve CBW curve (needed for BVWe)
        cbw_col = None
        for c in ["CBWapp", "CBW", "CBWa", "CBW_NMR", "CBWAPP"]:
            if c in df.columns:
                cbw_col = c
                break
        if cbw_col is None:
            print("[WS] Need CBW curve (CBWapp/CBW/CBWa/...) to compute BVWe_CP")
            return

    
        # --- Resolve VSH to build MSTAR (uses your same line relationship)
        vsh_col = "VSH_HL" if "VSH_HL" in df.columns else None
        if vsh_col is None:
            print("[WS] Need VSH_HL (compute Vsh first) to build MSTAR for Waxman-Smits")
            return
        ...
        '''
        # PHIE = PHIT - CBWapp (clip to >=0)
        #df["PHIE"] = np.clip(PHIT - cbw, 0.0, None)
        df["PHIE"] = pd.Series(np.clip(PHIT - cbw, 0.0, None), index=df.index)
        '''
        PHIE = PHIT - cbw
        PHIE = np.clip(PHIE, 0.0, None)
        PHIE = np.minimum(PHIE, PHIT)     # optional but usually physically consistent
        df["PHIE"] = PHIE
    ...
    def _build_nd_tab(self):
        ...
        btn_vsh = QPushButton("Compute Vsh (GR/ND/DT) + HL")
        ...
        for k, (mn, mx, st, dec) in spec.items():
            ...
            sb = make_dspin(mn, mx, st, dec, val)
            sb.valueChanged.connect(lambda v, kk=k: self._set_param_and_refresh(kk, float(v)))
            form.addRow(_pretty(k), sb)
    ...
    def _build_gr_tab(self):
        ...
        sb_clean = make_dspin(0, 300, 1, 0, gr_clean)
        sb_shale = make_dspin(0, 300, 1, 0, gr_shale)

        sb_clean.valueChanged.connect(lambda v: self._set_param_and_refresh("gr_clean", float(v)))
        sb_shale.valueChanged.connect(lambda v: self._set_param_and_refresh("gr_shale", float(v)))

        form.addRow("GR clean", sb_clean)
        form.addRow("GR shale", sb_shale)
    ...
    def _build_dt_tab(self):
        ...
        sb_dt_m = make_dspin(40, 120, 0.5, 1, dt_matrix)
        sb_dt_s = make_dspin(40, 140, 0.5, 1, dt_shale)
        sb_dt_f = make_dspin(100, 300, 1.0, 1, dt_fl)

        sb_dt_m.valueChanged.connect(lambda v: self._set_param_and_refresh("dt_matrix", float(v)))
        sb_dt_s.valueChanged.connect(lambda v: self._set_param_and_refresh("dt_shale", float(v)))
        sb_dt_f.valueChanged.connect(lambda v: self._set_param_and_refresh("dt_fl", float(v)))

        form.addRow("DT matrix (us/ft)", sb_dt_m)
        form.addRow("DT shale (us/ft)",  sb_dt_s)
        form.addRow("DT fluid (us/ft)",  sb_dt_f)

        for k, label, rng, dflt in [
            ("den_matrix", "RHOB matrix (g/cc)", (1.5, 3.0), 2.65),
            ("den_shale",  "RHOB shale (g/cc)",  (1.5, 3.0), 2.65),
            ("den_fl",     "RHOB fluid (g/cc)",  (0.8, 2.0), 1.10),
        ]:
            val = float(self.param_store.get(k, dflt))
            sb = make_dspin(rng[0], rng[1], 0.01, 3, val)
            sb.valueChanged.connect(lambda v, kk=k: self._set_param_and_refresh(kk, float(v)))
    ...
    def _build_sw_tab(self):
        ...
        m_cem  = float(ps.get("sw.m_cem", 2.0))
        n_sat  = float(ps.get("sw.n_sat", 2.0))
        rw     = float(ps.get("sw.rw", 0.03))
        mslope = float(ps.get("sw.mslope", 1.0))
        B      = float(ps.get("sw.B", 0.0))

        sb_m  = make_dspin(1.00, 3.00, 0.01, 2, m_cem)
        sb_n  = make_dspin(1.00, 3.00, 0.01, 2, n_sat)
        sb_rw = make_dspin(0.001, 0.20, 0.001, 4, rw)
        sb_ms = make_dspin(0.01, 4.00, 0.01, 2, mslope)
        sb_B  = make_dspin(0.0,  20.0, 0.1,  2, B)

        sb_m.valueChanged.connect(lambda v: self._set_param_and_refresh("sw.m_cem", float(v)))
        sb_n.valueChanged.connect(lambda v: self._set_param_and_refresh("sw.n_sat", float(v)))
        sb_rw.valueChanged.connect(lambda v: self._set_param_and_refresh("sw.rw", float(v)))
        sb_ms.valueChanged.connect(lambda v: self._set_param_and_refresh("sw.mslope", float(v)))
        sb_B.valueChanged.connect(lambda v: self._set_param_and_refresh("sw.B", float(v)))

        cbw_int = float(ps.get("cbw_intercept", 0.15))
        sb_cbw_int = make_dspin(0.0, 0.5, 0.01, 3, cbw_int)
        sb_cbw_int.valueChanged.connect(lambda v: self._set_param_and_refresh("cbw_intercept", float(v)))

        form.addRow("m (cementation)", sb_m)
        form.addRow("n (saturation)",  sb_n)
        form.addRow("Rw",              sb_rw)
        form.addRow("M* slope",        sb_ms)
        form.addRow("B (WS term)",     sb_B)
        form.addRow("CBW Intercept",   sb_cbw_int)

        btn_sw = QPushButton("Compute General Sw / BVW / M*")
        btn_sw.clicked.connect(self._on_compute_sw_clicked)
        form.addRow(btn_sw)

        btn_ws = QPushButton("Compute Waxman–Smits Sw")
    ...
    def update_depth_plot(self, state):
        ...
        tnph_curve = get_curve("tnph_curve", fallbacks=["NPOR", "TNPH", "NPHI", "CNL"])
        ...
        # Track 3: Rt (log x)
        # -------------------------
        plot("rt", arr(rt_curve), pg.mkPen("lightgray", width=2), xlim=(0.2, 20), invert_x=False, logx=False)
        pi.addLegend()
        ...
        # -------------------------
        # Track 5: BVW fills (PHIT_NMR, PHIE_NMR, BVIE)
        # -------------------------
        pi_nmr = self.track_items["bvw"]
        x_phit = arr(phit_nmr_curve)
        x_phie = arr(phie_nmr_curve)
        x_bvie = arr(bvie_curve)
        ...
        # Optional: overlay PHIT_CHART on NMR track
        if x_phit_chart is not None:
            pi_nmr.plot(x_phit_chart, depth, pen=pg.mkPen("y", width=2), name="PHIT_CHART")

        # -------------------------
        # Overlay BVWe_CP on NMR track (effective bulk water from Waxman-Smits)
        # -------------------------
        x_bvwe = get_col(df, "BVWe_CP")
        if x_bvwe is not None:
            m = np.isfinite(depth) & np.isfinite(x_bvwe)
            if np.any(m):
                d = depth[m].astype(float)
                bvwe = np.clip(x_bvwe[m].astype(float), 0.0, 0.30)  # display clamp to NMR scale
                pi_nmr.plot(bvwe, d, pen=pg.mkPen("cyan", width=3), name="BVWe_CP")

        # Overlay PHIE 

        x_phie_ws = get_col(df, "PHIE")
        if x_phie_ws is not None:
            m = np.isfinite(depth) & np.isfinite(x_phie_ws)
            if np.any(m):
                d = depth[m].astype(float)
                phie_ws = np.clip(x_phie_ws[m].astype(float), 0.0, 0.30)
                pi_nmr.plot(phie_ws, d, pen=pg.mkPen("red", width=3), name="PHIE (WS)")
    ...
    def update_nd_crossplot(self, state):
        ...
        if tnph and rhob and (tnph in df.columns) and (rhob in df.columns):
            ...
            y = pd.to_numeric(df[rhob], errors="coerce").to_numpy(float)
            m = np.isfinite(x) & np.isfinite(y)
            ax.scatter(x[m], y[m], s=6, alpha=0.6, label="Well data")
            ax.set_xlabel(tnph)
            ax.set_ylabel(rhob)
            ax.invert_yaxis()
        ...
        ax.plot([A[0], B[0], C[0], A[0]], [A[1], B[1], C[1], A[1]], lw=2, alpha=0.9, label="Shale triangle")
        ax.text(A[0], A[1], " Matrix", fontsize=9, va="center")
        ax.text(B[0], B[1], " Shale",  fontsize=9, va="center")
        ax.text(C[0], C[1], " Fluid",  fontsize=9, va="center")
    ...
    def update_gr_hist(self, state):
        ...
        g = g[np.isfinite(g)]
        if g.size:
            ax.hist(g, bins=60, alpha=0.85)
            ax.axvline(gr_clean, lw=2, linestyle="--", label=f"GR clean = {gr_clean:g}")
            ax.axvline(gr_shale, lw=2, linestyle="--", label=f"GR shale = {gr_shale:g}")
            ax.set_xlabel(gr)
            ax.set_ylabel("Count")
            ax.grid(True, alpha=0.25)
            ax.legend(loc="best", fontsize=9)
