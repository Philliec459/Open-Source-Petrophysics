...
class MainController:
    ...
    def get_view_df(self):
        df = getattr(self.state, "view_df", None)
        if df is not None and not df.empty:
            return df
        return getattr(self.state, "analysis_df", None)

    def set_center_widget(self, widget, title=None):
        if self.main_window is None:
            raise AttributeError("MainController has no main_window attached")

        if hasattr(self.main_window, "set_center_widget"):
            return self.main_window.set_center_widget(widget, title)

        raise AttributeError("Main window has no set_center_widget method")



    def update_analysis_df(self, df):
        if df is None:
            return

        self.state.analysis_df = df
        self.rebuild_view()
