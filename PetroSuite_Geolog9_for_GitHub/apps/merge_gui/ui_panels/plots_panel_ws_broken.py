...
class PlotsPanelWS(QWidget):
    ...
    def _on_compute_sw_clicked(self):
        ...
        # ------------------------------------------------------------
        # Use the actual selected curves, not helper curves
        # ------------------------------------------------------------
        rt_col = params.get("rt_curve")
        if (not rt_col) or (rt_col not in df.columns):
            rt_col = _first_present(df.columns, ["AT90", "AF90", "AO90", "ILD", "RLA5", "RT"])
        ...
        if vsh_col is None:
            print("[SW] Need VSH_HL to compute interactive SW/M*")
            return
        ...
        # Build arrays
        # ------------------------------------------------------------
        Rt = pd.to_numeric(df[rt_col], errors="coerce").to_numpy(dtype=float)
        PHIT = pd.to_numeric(df[phit_col], errors="coerce").to_numpy(dtype=float)
        Vsh = pd.to_numeric(df[vsh_col], errors="coerce").to_numpy(dtype=float)
    
        Rt = np.clip(Rt, 1e-6, None)
        ...
        # ------------------------------------------------------------
        # Interactive HSK path: CBWapp -> SWB -> Qv
        # ------------------------------------------------------------
        ...
        denom_hsk = (0.6425 / np.sqrt(max(den_fl * SAL, 1e-12)) + 0.22)
        Qv = np.clip(SWB / denom_hsk, 0.0, 5.0)
        df["Qv"] = Qv
    
        # ------------------------------------------------------------
        # MSTAR model and apparent MSTAR
        # ------------------------------------------------------------
        MSTAR = Vsh * mslope + m_cem
        df["MSTAR_FIT"] = MSTAR
    
        PHIT_app = np.clip(PHIT, 1e-6, 0.999)
        Rt_safe = np.clip(Rt, 1e-6, None)
        Qv_safe = np.clip(Qv, 0.0, None)
    
        term = rw / (Rt_safe * (1.0 + rw * B * Qv_safe))
        term = np.clip(term, 1e-12, None)
    
        MSTAR_APP = np.log10(term) / np.log10(PHIT_app)
        MSTAR_APP = np.where(np.isfinite(MSTAR_APP), MSTAR_APP, np.nan)
        df["MSTAR_APP"] = MSTAR_APP
    
        # ------------------------------------------------------------
        # Main Archie-style interactive products
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Merge ZoI/view results back into full dataframe
        # ------------------------------------------------------------
        cols_to_merge = [
            "BVW",
            "SWT",
        ...
        print(
            "[SW] Interactive Archie/HSK update complete | "
            f"BVW non-null={int(pd.Series(df['BVW']).notna().sum())}, "
            f"MSTAR_APP non-null={int(pd.Series(df['MSTAR_APP']).notna().sum())}"
        )       
    ...
    def _on_compute_ws_clicked(self):
        ...
        try:
            ...
            good = np.isfinite(Rt) & np.isfinite(PHIT) & np.isfinite(MSTAR) & (Rt > 0) & (PHIT > 0)
        ...
        # ------------------------------------------------------------
        df["SW_WS"] = Sw_cp
        df["BVWT_WS"] = PHIT * Sw_cp
        df["BVWE_WS"] = np.clip(PHIT * Sw_cp - cbw, 0.0, None)
        ...
        print("[WS] Final Waxman-Smits calculation complete")
    ...
    def update_sw_tab(self, state):
        ...
        if top is not None and base is not None:
            ...
            mask = np.isfinite(depth) & (depth >= min(top, base)) & (depth <= max(top, base))
            if np.any(mask):
                df = df.loc[mask].copy()
        

    
        # optional: trim to ZoI if depth_top/depth_base exist
        top = getattr(state, "depth_top", None)
        base = getattr(state, "depth_base", None)
        if top is not None and base is not None:
            depth = self._depth_array(df)
