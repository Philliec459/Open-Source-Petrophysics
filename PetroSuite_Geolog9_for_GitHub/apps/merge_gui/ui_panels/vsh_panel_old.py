...
VSH_PARAM_SPEC = {
    "vsh.gr_clean": dict(default=20.0, mn=0.0, mx=200.0, step=1.0, dec=1, label="GR clean"),
    "vsh.gr_shale": dict(default=120.0, mn=0.0, mx=300.0, step=1.0, dec=1, label="GR shale"),
    ...
    "vsh.dt_shale": dict(default=110.0, mn=40.0, mx=220.0, step=1.0, dec=1, label="DT shale"),
...
class VshPanel(QWidget):
    ...
    # ------------------------------------------------------------------
    def _build_ui(self):
        from PySide6.QtWidgets import QGroupBox, QFormLayout, QVBoxLayout, QHBoxLayout
    ...
    def _build_ui(self):
        ...
        param_form.setLabelAlignment(Qt.AlignRight | Qt.AlignVCenter)
        param_form.setFormAlignment(Qt.AlignTop)
        param_form.setRowWrapPolicy(QFormLayout.DontWrapRows)
        ...
        for key, spec in VSH_PARAM_SPEC.items():
            ...
            w.setMinimumWidth(120)   # try 110 to 140
        ...
        status_layout.addWidget(self.notes)
    ...
    def calculate_vsh(self):
        ...
        # parameters
        gr_clean = float(params.get("vsh.gr_clean", 20.0))
        gr_shale = float(params.get("vsh.gr_shale", 120.0))
        ...
        dt_shale = float(params.get("vsh.dt_shale", 110.0))
        ...
        summary.append(f"NPHI shale = {nphi_shale:.3f}")
        summary.append("")
        summary.append(f"Valid VSH points: {int(pd.to_numeric(out['VSH'], errors='coerce').notna().sum())}")
        ...
        if hasattr(self.controller, "refresh_plots"):
            self.controller.refresh_plots()
        elif hasattr(self.controller, "refresh_ui"):
            self.controller.refresh_ui()