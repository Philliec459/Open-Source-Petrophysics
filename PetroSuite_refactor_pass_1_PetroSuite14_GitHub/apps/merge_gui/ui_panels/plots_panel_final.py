...
import numpy as np
import pandas as pd
from petrocore.config.curve_aliases import family_candidates
...
class PlotsPanelFinal(QWidget):
    ...
    def normalize_nmr_curve(self, x):
        if x is None:
            return None
        ...
        try:
            ...
            mean_val = np.nanmean(x_valid)
    
            # If mean > 1 → assume percent → convert to fraction
            if mean_val > 1.5:
                return x / 100.0
    ...
    def _refresh_final_depth_plot(self):
        ...
        gr_curve = self._get_curve_name(df, params, "gr_curve", fallbacks=family_candidates("GR"))
        cgr_curve = self._get_curve_name(df, params, "cgr_curve", fallbacks=family_candidates("CGR"))
        tnph_curve = self._get_curve_name(df, params, "tnph_curve", fallbacks=family_candidates("TNPH"))
        rhob_curve = self._get_curve_name(df, params, "rhob_curve", fallbacks=family_candidates("RHOB"))
        rt_curve = self._get_curve_name(df, params, "rt_curve", fallbacks=["AT90", "AF90", "AO90", "ILD", "RT"])
        phit_nmr_curve = self._get_curve_name(df, params, "tcmr_curve", fallbacks=["TCMR","MPHS","MPHI", "PHIT_NMR", "MPHS"])
        phie_nmr_curve = self._get_curve_name(df, params, "cmrp_curve", fallbacks=["CMRP_3MS","MBVI", "PHIE_NMR",  "CMRP3MS", "CMRP", "MPHI"])
        bvie_curve = self._get_curve_name(df, params, "bvie_curve", fallbacks=["BVIE", "MBVI","BVI_E", "BVI", "MBVI"])
        ...
        if vsh is not None:
            self._plot_track_curve(items, "gr", depth, vsh * 200.0, pg.mkPen("orange", width=4), xlim=(0, 200), name="VSH_HL*200")
        ...
        #
        #
        #  watch /100 for MRIL data
        # ---------------------------------------------------------------------
        self._plot_track_curve(items, "por", depth, arr(tnph_curve), pg.mkPen("g", width=1), xlim=(-0.15, 0.45), invert_x=True, name=tnph_curve)
        #self._plot_track_curve(items, "por", depth, arr(phit_nmr_curve)/100, pg.mkPen("m", width=1), xlim=(-0.15, 0.45), invert_x=True, name=phit_nmr_curve)
        #self._plot_track_curve(items, "por", depth, arr(phie_nmr_curve)/100, pg.mkPen("b", width=1), xlim=(-0.15, 0.45), invert_x=True, name=phie_nmr_curve)

        self._plot_track_curve(items, "por", depth, self.normalize_nmr_curve(arr(phit_nmr_curve)),
                               pg.mkPen("m", width=1), xlim=(-0.15, 0.45), invert_x=True, name=phit_nmr_curve)

        self._plot_track_curve(items, "por", depth, self.normalize_nmr_curve(arr(phie_nmr_curve)),
                               pg.mkPen("b", width=1), xlim=(-0.15, 0.45), invert_x=True, name=phie_nmr_curve)
        ...
        #x_phit_nmr = arr(phit_nmr_curve)/100
        x_phie_nmr = arr(phie_nmr_curve)
        #x_bvie = arr(bvie_curve)/100
        #x_bfv = arr(bfv_curve)


        x_phit_nmr = self.normalize_nmr_curve(arr(phit_nmr_curve))
        #x_phie_nmr = self.normalize_nmr_curve(arr(phie_nmr_curve))
        x_bvie = self.normalize_nmr_curve(arr(bvie_curve))
        x_bfv = self.normalize_nmr_curve(arr(bfv_curve))
        ...
        if (x_phit_nmr is not None) and (x_phie_nmr is not None) and (x_bvie is not None):
            ...
            if np.any(m):
                ...
                phitn = np.clip(np.asarray(x_phit_nmr[m], dtype=float), 0.0, nmr_track_max)
                phien = np.clip(np.asarray(x_phie_nmr[m], dtype=float), 0.0, nmr_track_max)
                phien = np.minimum(phien, phitn)
        
                bvien = np.clip(np.asarray(x_bvie[m], dtype=float), 0.0, nmr_track_max)
                ...
                pi_nmr.setXRange(0.0, nmr_track_max, padding=0.0)
        ...
        if x_phit is not None:
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phit = np.clip(x_phit[m].astype(float), 0.0, bvw_track_max)
        ...
        if (x_phit is not None) and (x_phie is not None):
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phit = np.clip(x_phit[m].astype(float), 0.0, bvw_track_max)
                phie = np.minimum(np.clip(x_phie[m].astype(float), 0.0, bvw_track_max), phit)
        ...
        if x_bvw is not None:
            if x_phie is not None:
                ...
                if np.any(m):
                    d = depth[m].astype(float)
                    phie = np.clip(x_phie[m].astype(float), 0.0, bvw_track_max)
                    bvw = np.minimum(np.clip(x_bvw[m].astype(float), 0.0, bvw_track_max), phie)
                ...
                if np.any(m):
                    d = depth[m].astype(float)
                    bvw = np.clip(x_bvw[m].astype(float), 0.0, bvw_track_max)
                    c_bvw = pi_bvw.plot(bvw, d, pen=pg.mkPen("dodgerblue", width=2), name=bvw_name)
                    zero = np.zeros_like(bvw)
                    c_zero = pi_bvw.plot(zero, d, pen=pg.mkPen("w", width=1), name="0")
        ...
        if (x_bvie is not None):
                c_bvie = pi_bvw.plot(bvien, d, pen=pg.mkPen("b", width=1), name="BVIE")
                zero = np.zeros_like(bvien)
                c_zero = pi_bvw.plot(zero, d, pen=pg.mkPen("w", width=1), name="0")
                pi_bvw.addItem(pg.FillBetweenItem(c_bvie, c_zero, brush=pg.mkBrush("b")))


        pi_bvw.getViewBox().invertX(True)
        pi_bvw.setXRange(0.0, bvw_track_max, padding=0.0)
    ...
    def update_depth_plot(self, state):
        ...
        if (x_phit is not None) and (x_phie is not None) and (x_bvie is not None):
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phit = np.clip(x_phit[m].astype(float), 0.0, nmr_track_max)
                phie = np.minimum(np.clip(x_phie[m].astype(float), 0.0, nmr_track_max), phit)
                bvie = np.minimum(np.clip(x_bvie[m].astype(float), 0.0, nmr_track_max), phie)
