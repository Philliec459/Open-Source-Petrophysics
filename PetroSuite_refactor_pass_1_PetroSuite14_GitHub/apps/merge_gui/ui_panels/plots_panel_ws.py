...
SW_PARAM_SPEC = {
    ...
    "sw.rw": dict(default=0.03, mn=0.001, mx=0.20, step=0.001, dec=4, label="Rw"),
...
class PlotsPanelWS(QWidget):
    ...
    def _on_compute_sw_clicked(self):
        ...
        qv_col = "Qv"
        Qv = pd.to_numeric(df[qv_col], errors="coerce").to_numpy(dtype=float)
    ...
    def _on_compute_ws_clicked(self):
        ...
        Rt = pd.to_numeric(df[rt_col], errors="coerce").to_numpy(float)
        Qv = pd.to_numeric(df["Qv"], errors="coerce").to_numpy(float)
        ...
        # ==========================================================
        # ✅ NEW CORE SOLVER (THIS IS THE ONLY REAL CHANGE)
        # ==========================================================
        from petrocore.workflow.sw import compute_sw_waxman_smits
    
        try:
            Sw_cp = compute_sw_waxman_smits(
                Rt=Rt,
                phi=PHIT,
                Rw=rw,
                B=B,
    ...
    def update_sw_tab(self, state):
        ...
        if bvw is not None:
            ...
            axL.fill_betweenx(
                ...
                color="green", alpha=0.85, label="BVO",
            )
            axL.fill_betweenx(
                y, bvw, 0,
                where=np.isfinite(bvw),
                color="cyan", alpha=0.85, label="BVW fill",
            )
