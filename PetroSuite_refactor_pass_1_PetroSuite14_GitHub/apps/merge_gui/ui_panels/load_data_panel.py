...
class LoadDataPanel(QWidget):
    ...
    def _get_default_data_dir(self) -> str:
        ...
        # 1) Best case: full project_dir already stored in state
        ...
        # 2) Fallback: rebuild from project_name under PetroSuite_Projects
        project_name = getattr(state, "project_name", None)
        if project_name:
            base_dir = Path.cwd() / "PetroSuite_Projects" / str(project_name)
    ...
    def _on_load_las(self):
        ...
        start_dir = self._get_default_data_dir()
        self._append_info(f"LAS browser start folder: {start_dir}")
        ...
        if state.las_runs:
            ...
            try:
                ...
                state.params["hidden_curves"] = auto_pick_hidden_curves(curve_names)
    ...
    def _on_load_tops(self):
        ...
        start_dir = self._get_default_data_dir()
        self._append_info(f"Tops browser start folder: {start_dir}")
