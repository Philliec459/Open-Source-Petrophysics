...
from apps.merge_gui.ui_panels.xplot_panel import XPlotPanel
#from apps.merge_gui.ui_panels.crossplot_view import CrossplotView
from petrocore.workflow.crossplot_view import CrossplotView
from apps.merge_gui.widgets.altair_crossplot_view import AltairCrossplotView
...
from apps.merge_gui.plugins.plugin_manager import PluginManager
...
NAV_GROUPS = {
    ...
    "Load Previously Saved Wells": [
        ("Open Project/Wells", "open_project"),
    ...
    "Final Interactive Petrophysics": [
...
ICON_MAP = {
    ...
    "open_project": ["nav/load_data.svg", "load_data.svg", "rig.png"],
...
PRESET_MAP = {
    "Neutron-Density": ("NPHI", "RHOB"),
    "Sonic-Neutron": ("DTCO", "NPHI"),
    "Sonic-Density": ("DTCO", "RHOB"),
    "PEF-Bulk Density": ("PEF", "RHOB"),
    "UMAA-RHOMAA": ("UMAA", "RHOMAA"),
    "Pickett Plot": ("PHIT", "RT"),
    "Buckles Plot": ("PHIT", "SW"),
    "Steiber Plot": ("VSH", "PHIT"),
}
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.altair_crossplot_view = QWidget()
        self.altair_crossplot_layout = QVBoxLayout(self.altair_crossplot_view)
        self.altair_crossplot_layout.setContentsMargins(0, 0, 0, 0)

        self.stack.addWidget(self.altair_crossplot_view)
        ...
        #self.center_tabs.addTab(self.crossplot_view, "Crossplot")
        #self.center_tabs.addTab(self.altair_crossplot_view, "Altair Xplot")
    ...
    def set_center_widget(self, widget, title=""):
        if hasattr(self, "_plugin_center_widget") and self._plugin_center_widget is not None:
            try:
                self.stack.removeWidget(self._plugin_center_widget)
            except Exception:
                pass
            self._plugin_center_widget.deleteLater()
            self._plugin_center_widget = None

        self._plugin_center_widget = widget
    ...
    def _build_ui(self):
        ...
        self.crossplot_view = CrossplotView()
        self.stack.addWidget(self.crossplot_view)
        ...
        # ==============================================================
        # TAB 1: Traditional Workflow
        # ==============================================================
        self.workflow_tab = QWidget()
        self.workflow_tab_layout = QVBoxLayout(self.workflow_tab)
        self.workflow_tab_layout.setContentsMargins(0, 0, 0, 0)
        self.workflow_tab_layout.setSpacing(4)

        self.workflow_group = QGroupBox("Traditional Workflow")
        ...
        self.workflow_buttons_layout.setSpacing(6)
        ...
        self.workflow_scroll.setWidget(self.workflow_buttons_host)

        self.workflow_layout.addWidget(self.workflow_scroll)
        self.workflow_buttons_layout.addStretch()

        self.workflow_tab_layout.addWidget(self.workflow_group)

        # ==============================================================
        # TAB 2: Plugins
        # ==============================================================
        self.plugins_tab = QWidget()
        self.plugins_tab_layout = QVBoxLayout(self.plugins_tab)
        self.plugins_tab_layout.setContentsMargins(0, 0, 0, 0)
        self.plugins_tab_layout.setSpacing(4)

        self.plugins_group = QGroupBox("Plugins")
        self.plugins_layout = QVBoxLayout(self.plugins_group)
        self.plugins_layout.setContentsMargins(6, 6, 6, 6)
        self.plugins_layout.setSpacing(6)

        self.plugin_buttons_host = QWidget()
        self.plugin_buttons_layout = QVBoxLayout(self.plugin_buttons_host)
        ...
        self.plugins_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.plugins_scroll.setWidget(self.plugin_buttons_host)
        ...
        for plugin in self.plugins:
            ...
            btn.setToolTip(getattr(plugin, "tooltip", ""))

            btn.clicked.connect(
                lambda checked=False, p=plugin: p.launch(self.controller)
            )
        ...
        self.plugins_tab_layout.addWidget(self.plugins_group)
        ...
        # ---- Add tabs ------------------------------------------------
        self.right_tabs.addTab(self.workflow_tab, "Workflow")

        self.right_tabs.addTab(self.plugins_tab, "Plugins")

        self.xplot_panel = XPlotPanel(main_window=self)
        self.right_tabs.addTab(self.xplot_panel, "Xplot")
    ...
    def _build_panels(self):
        ...
        ordered_panels = [
            ...
            ("Open Project/Wells", self.open_project_panel),
    ...
    def launch_xplot(self, name):
        print(f"Launching xplot: {name}")

        if name == "Altair Xplot":
            self.launch_altair_xplot()
            return

        df_plot = self.get_crossplot_df_from_selected_tops()
        if df_plot is None:
            print("[launch_xplot] df_plot is None, aborting")
            return

        if df_plot.empty:
            print("[launch_xplot] df_plot is empty, aborting")
            return

        print(f"[launch_xplot] using df with shape: {df_plot.shape}")


        self.crossplot_view.load_preset(name, df_plot)
        
        # --------------------------------------------------
        # Apply YAML winners automatically
        # --------------------------------------------------
        winners = getattr(self, "_yaml_winners_map", {})
        
        families = PRESET_MAP.get(name)
        
        if winners and families:
            print("[XPlot] applying YAML winners")
        
            x_family, y_family = families
        
            x_curve = winners.get(x_family)
            y_curve = winners.get(y_family)
        
            if hasattr(self.crossplot_view, "x_combo") and x_curve in df_plot.columns:
                print(f"[XPlot] X → {x_curve}")
                self.crossplot_view.x_combo.setCurrentText(x_curve)
        
            if hasattr(self.crossplot_view, "y_combo") and y_curve in df_plot.columns:
                print(f"[XPlot] Y → {y_curve}")
                self.crossplot_view.y_combo.setCurrentText(y_curve)
        ...
        if hasattr(self, "stack"):
            self.stack.setCurrentWidget(self.crossplot_view)    

        
    def get_crossplot_df_from_selected_tops(self):
        """
        Filter dataframe using ZoI already stored in state.
        """
    
        # get dataframe
        
        # try merged first
        df = getattr(self.state, "merged_df", None)
        
        # fallback to analysis / working
        if df is None:
            df = getattr(self.state, "analysis_df", None)
        
        if df is None:
            df = getattr(self.state, "working_df", None)
        
        # final fallback → raw runs
        if df is None:
            runs = getattr(self.state, "las_runs", None) or getattr(self.state, "raw_runs", None)
            if runs and len(runs) > 0:
                df = getattr(runs[0], "df", None)
        
        # still nothing → abort
        if df is None:
            print("[crossplot] no dataframe found anywhere")
            return None
        ...
        # detect depth column
        depth_col = None
        for c in ["DEPTH", "DEPT", "Depth", "MD", "TVD", "Z"]:
            if c in df.columns:
                depth_col = c
                break
    
        if depth_col is None:
            print("[crossplot] no depth column found")
            return df
    
        print(f"[crossplot] using depth column: {depth_col}")
    
        # get ZoI from state
        z1 = getattr(self.state, "zoi_top", None)
        z2 = getattr(self.state, "zoi_base", None)
    
        if z1 is None or z2 is None:
            print("[crossplot] no ZoI selected → using full dataframe")
            return df
    
        try:
            zmin, zmax = sorted([float(z1), float(z2)])
        except Exception:
            print("[crossplot] invalid ZoI → using full dataframe")
            return df
    
        print(f"[crossplot] filtering between {zmin} and {zmax}")
    
        return df[(df[depth_col] >= zmin) & (df[depth_col] <= zmax)].copy()    
    ...
    def launch_altair_xplot(self):
        print("Launching Altair Xplot Dashboard")
    
        df_plot = self.get_crossplot_df_from_selected_tops()
        if df_plot is None:
            print("[launch_altair_xplot] df_plot is None, aborting")
            return
    
        if df_plot.empty:
            print("[launch_altair_xplot] df_plot is empty, aborting")
            return
    
        print(f"[launch_altair_xplot] using df with shape: {df_plot.shape}")
        print("build_altair_dashboard from:", build_altair_dashboard.__module__)
        ...
        try:
            ...
            out_file = Path("petro_suite_altair_xplot.html").resolve()
            chart.save(str(out_file))
            webbrowser.open(out_file.as_uri())
    
        except Exception as e:
            print(f"[launch_altair_xplot] failed: {e}")                
