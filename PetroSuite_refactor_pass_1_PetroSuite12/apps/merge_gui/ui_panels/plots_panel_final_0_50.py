    ...
    def _refresh_final_depth_plot(self):
        ...
        if (x_phit_nmr is not None) and (x_phie_nmr is not None) and (x_bvie is not None):
            ...
            if np.any(m):
                ...
                phitn = np.clip(np.asarray(x_phit_nmr[m], dtype=float), 0.0, 0.50)
                phien = np.clip(np.asarray(x_phie_nmr[m], dtype=float), 0.0, 0.50)
                phien = np.minimum(phien, phitn)
        
                bvien = np.clip(np.asarray(x_bvie[m], dtype=float), 0.0, 0.50)
                ...
                pi_nmr.setXRange(0.0, 0.50, padding=0.0)
        ...
        if x_phit is not None:
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phit = np.clip(x_phit[m].astype(float), 0.0, 0.50)
        ...
        if (x_phit is not None) and (x_phie is not None):
            ...
            if np.any(m):
                d = depth[m].astype(float)
                phit = np.clip(x_phit[m].astype(float), 0.0, 0.50)
                phie = np.minimum(np.clip(x_phie[m].astype(float), 0.0, 0.50), phit)
        ...
        if x_bvw is not None:
            if x_phie is not None:
                ...
                if np.any(m):
                    d = depth[m].astype(float)
                    phie = np.clip(x_phie[m].astype(float), 0.0, 0.50)
                    bvw = np.minimum(np.clip(x_bvw[m].astype(float), 0.0, 0.50), phie)
                ...
                if np.any(m):
                    d = depth[m].astype(float)
                    bvw = np.clip(x_bvw[m].astype(float), 0.0, 0.50)
                    c_bvw = pi_bvw.plot(bvw, d, pen=pg.mkPen("dodgerblue", width=2), name=bvw_name)
                    zero = np.zeros_like(bvw)
                    c_zero = pi_bvw.plot(zero, d, pen=pg.mkPen("w", width=1), name="0")
