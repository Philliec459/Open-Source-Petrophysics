...
class SaveWellPanel(QWidget):
    ...
    def _prefill_from_state(self):
        ...
        project_name = getattr(state, "project_name", "") or "Statoil"
    ...
    def save_well(self):
        ...
        try:
            ...
            # ----------------------------------------------------------
            # Tops
            # Prefer current_tops_df because it reflects the active tops
            # currently in use for this well.
            # ----------------------------------------------------------
            ...
            zoi_top = getattr(state, "zoi_top", None)
            zoi_base = getattr(state, "zoi_base", None)
            zoi_depth_range = getattr(state, "zoi_depth_range", None)

            if not zoi_depth_range and zoi_top is not None and zoi_base is not None:
                try:
                    zoi_depth_range = [float(zoi_top), float(zoi_base)]
                except Exception:
                    zoi_depth_range = None

            params["zoi.top"] = zoi_top
            params["zoi.base"] = zoi_base
            params["zoi_depth_range"] = zoi_depth_range
            params["zoi_top_name"] = getattr(state, "zoi_top_name", None)
            params["zoi_base_name"] = getattr(state, "zoi_base_name", None)

            params["zoi_top_raw"] = getattr(state, "zoi_top_raw", None)
            params["zoi_base_raw"] = getattr(state, "zoi_base_raw", None)
            params["zoi_above_pad"] = getattr(state, "zoi_above_pad", None)
            params["zoi_below_pad"] = getattr(state, "zoi_below_pad", None)
            ...
            if zoi_top is not None:
                setattr(state, "zoi_top", zoi_top)
            if zoi_base is not None:
                setattr(state, "zoi_base", zoi_base)
            if zoi_depth_range is not None:
                setattr(state, "zoi_depth_range", zoi_depth_range)
            ...
            if zoi_depth_range:
                msg.append(f"ZoI range: {zoi_depth_range}")
